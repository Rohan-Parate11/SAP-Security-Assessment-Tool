...
CHECK = CheckSpec(
    ...
    risk_tiles=[
        RiskTile("Users holding SAP_ALL", "crit", lambda s: s.get("user_count") or None),
    ],
)
...
def collect(connector: SAPConnector, config: dict) -> CheckResult:
    ...
    findings = users_with_profile(connector, "SAP_ALL")
    ...
    return CheckResult(
        ...
        summary={
            ...
            "note": "removal_candidates are users holding SAP_ALL who are unlocked but haven't "
            f"logged on in over {dormant_days} days -- the most defensible first group to review "
            "for removing SAP_ALL from.",
        },
    )
